  ...
  switch (req.method) {
    case "GET":
      try {
        ...
        // Retourner les données du film et les commentaires
        res.status(200).json({ status: 200, data: { movie: dbMovie, comments: comments } });
      } catch (error) {
        res.status(500).json({ error: "Erreur lors de la récupération des données du film et des commentaires" });
      }
      break;
